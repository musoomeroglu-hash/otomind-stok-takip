...
import type { Product, CustomOrder, Material, CariAccount, CariTransaction, Supplier, Sale, Purchase, CashEntry, Reminder, CalendarEvent, CustomMaterialType, WhatsappConfig, CashRegister } from '../types';
...
interface DataContextType {
  ...
  addCashRegister: (name: string) => Promise<void>;
  updateCashRegister: (id: string, name: string) => Promise<void>;
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      try {
        const [p, co, m, a, ct, s, sa, pu, ce, r, calEv, mt, cr] = await Promise.all([
          ...
          sbFetchAll<CashRegister>(DB_TABLES.cash_registers),
        ]);
  ...
  // --- CASH REGISTERS ---
  const addCashRegister = useCallback(async (name: string) => {
    ...
    if (isSupabaseConfigured()) {
      await sbInsert(DB_TABLES.cash_registers, mapToDb(newRegister));
    }
    setCashRegisters(prev => {
      const next = [...prev, newRegister];
      if (!isSupabaseConfigured()) localStorage.setItem('otomind_cashRegisters', JSON.stringify(next));
      return next;
    });
  }, []);

  const updateCashRegister = useCallback(async (id: string, name: string) => {
    if (isSupabaseConfigured()) {
      await sbUpdate(DB_TABLES.cash_registers, id, mapToDb({ name }));
    }
    setCashRegisters(prev => {
  ...
  const deleteCashRegister = useCallback(async (id: string) => {
    // "Önceki Kasa" silinemez
    if (id === 'default_onceki_kasa') return;
    if (isSupabaseConfigured()) {
      await sbDelete(DB_TABLES.cash_registers, id);
    }
    setCashRegisters(prev => {
  ...
  const addCashEntry = useCallback(async (e: Omit<CashEntry, 'id' | 'createdAt'>) => {
    const newItem: CashEntry = { ...e, cashRegisterId: e.cashRegisterId || 'default_onceki_kasa', id: uid(), createdAt: new Date().toISOString() };
  ...
  return (
    <DataContext.Provider value={{
      products, customOrders, materials, accounts, cariTransactions, suppliers, sales, purchases, cashEntries, reminders, calendarEvents, materialTypes, cashRegisters, whatsappConfig, firmaBilgileri, updateFirmaBilgileri, usdRate, loading,
