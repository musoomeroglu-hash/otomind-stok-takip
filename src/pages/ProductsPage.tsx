      ...
      try {
        ...
        rows.forEach(row => {
          ...
          addProduct({
            ...
            locationSira: Number(row['Sıra No']) || undefined,
            locationCivi: Number(row['Çivi No']) || undefined,
            notes: row['Notlar'] || '',
            isCustom: false
          });
          added++;
                  ...
                  <tr key={p.id} className="border-b border-white/[0.04] hover:bg-overlay-border-light">
                    <td className="px-4 py-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-dark">{p.fabricType} {p.color && `• ${p.color}`}{(p.locationSira || p.locationCivi) && (<span className="ml-2 text-cyan-400">📍 {p.locationSira ? `${p.locationSira}. Sıra` : ''}{p.locationSira && p.locationCivi ? ' / ' : ''}{p.locationCivi ? `${p.locationCivi}. Çivi` : ''}</span>)}</p>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {(p.locationSira || p.locationCivi) ? (
                        <div className="flex items-center gap-1">
                          <span className="material-symbols-outlined text-cyan-400 text-[14px]">location_on</span>
                          <span className="text-xs text-cyan-300 font-medium">
                            {p.locationSira ? `${p.locationSira}. Sıra` : ''}
                            {p.locationSira && p.locationCivi ? ' — ' : ''}
                            {p.locationCivi ? `${p.locationCivi}. Çivi` : ''}
                          </span>
                        </div>
                      ) : (
      ...
      >
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-main text-sm font-medium">Depo Konumu</h3>
            <span className="text-muted-dark text-[10px] ml-1">(Opsiyonel)</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-muted mb-1 block">Sıra Numarası</label>
              <input type="number" min="1" value={form.locationSira ?? ''} onChange={e => setForm(f => ({ ...f, locationSira: e.target.value ? Number(e.target.value) : undefined }))} className="input-field w-full" placeholder="Örn: 3" />
            </div>
            <div>
              <label className="text-xs text-muted mb-1 block">Çivi Numarası</label>
              <input type="number" min="1" value={form.locationCivi ?? ''} onChange={e => setForm(f => ({ ...f, locationCivi: e.target.value ? Number(e.target.value) : undefined }))} className="input-field w-full" placeholder="Örn: 5" />
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-overlay-light border border-divider-light rounded-xl p-4">
              ...
              <div className="ml-7 flex flex-wrap gap-2">
                ...
                <span className="text-xs bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 px-2 py-1 rounded-lg">Sıra No</span>
                <span className="text-xs bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 px-2 py-1 rounded-lg">Çivi No</span>
